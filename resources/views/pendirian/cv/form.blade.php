...
<head>
    ...
    <style>
        ...
        :root {
            --primary: #0b326b; --primary-foreground: #ffffff;
            --accent: #0077e6; --accent-foreground: #ffffff; --accent-hover: #0060ba;
            ...
            --radius: 0.75rem; --touch-target: 44px;
        }
        body { font-family: 'Inter', sans-serif; background-color: var(--muted); color: var(--foreground); margin: 0; padding-bottom: 90px; }
        header.app-header { background-color: var(--primary); color: var(--primary-foreground); padding: 1rem; box-shadow: var(--shadow-md); }
        .stepper-sticky { background-color: var(--card); border-bottom: 1px solid var(--border); box-shadow: var(--shadow-sm); }
        .sticky-top-wrapper { position: sticky; top: 0; z-index: 50; width: 100%; }
        .stepper-content { max-width: 42rem; margin: 0 auto; padding: 1rem; }
        .desktop-stepper { display: none; position: relative; justify-content: space-between; align-items: center; }
        @media (min-width: 768px) { .desktop-stepper { display: flex; } }
        .stepper-line-bg { position: absolute; left: 0; right: 0; top: 1.25rem; height: 2px; background-color: var(--muted); z-index: 0; border-radius: 999px; }
        .stepper-line-fill { position: absolute; left: 0; top: 1.25rem; height: 2px; background-color: var(--accent); z-index: 0; transition: width 0.5s ease-out; width: 0%; }
        .step-item { position: relative; z-index: 10; display: flex; flex-direction: column; align-items: center; gap: 0.5rem; min-width: 80px; }
        .step-circle { width: 2.5rem; height: 2.5rem; border-radius: 50%; background-color: var(--muted); color: var(--muted-foreground); display: flex; align-items: center; justify-content: center; transition: all 0.3s ease; box-shadow: 0 0 0 4px var(--card); }
        .step-item.active .step-circle { background-color: var(--primary); color: var(--primary-foreground); transform: scale(1.1); box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.1), 0 0 0 4px var(--card); }
        .step-item.completed .step-circle { background-color: var(--accent); color: var(--accent-foreground); box-shadow: var(--shadow-sm), 0 0 0 4px var(--card); }
        .step-label { font-size: 0.75rem; font-weight: 500; color: var(--muted-foreground); transition: color 0.3s; }
        .step-item.active .step-label, .step-item.completed .step-label { color: var(--foreground); }
        .mobile-stepper { display: block; }
        @media (min-width: 768px) { .mobile-stepper { display: none; } }
        .mobile-stepper-header { display: flex; justify-content: space-between; margin-bottom: 0.5rem; font-size: 0.875rem; font-weight: 500; }
        .mobile-progress-track { height: 0.5rem; background-color: var(--muted); border-radius: 999px; overflow: hidden; }
        .mobile-progress-fill { height: 100%; background-color: var(--accent); transition: width 0.5s ease; width: 20%; }
        .card-elevated { background-color: var(--card); border-radius: var(--radius); box-shadow: var(--shadow-md); border: 1px solid var(--border); animation: slide-in 0.4s ease-out; }
        .form-label { font-size: 0.875rem; font-weight: 500; color: var(--foreground); margin-bottom: 0.5rem; display: block; }
        .form-input { width: 100%; min-height: var(--touch-target); padding: 0.5rem 0.75rem; font-size: 0.875rem; border-radius: 0.5rem; border: 1px solid var(--border); color: var(--foreground); background-color: white; transition: all 0.2s; }
        .form-input:focus { border-color: var(--primary); box-shadow: 0 0 0 2px rgba(11, 50, 107, 0.1); }
        .bottom-nav { background-color: var(--card); border-top: 1px solid var(--border); padding: 1rem; position: fixed; bottom: 0; left: 0; right: 0; z-index: 50; box-shadow: 0 -4px 6px -1px rgba(0, 0, 0, 0.05); }
        .btn { display: inline-flex; align-items: center; justify-content: center; border-radius: 0.5rem; font-weight: 500; font-size: 0.875rem; height: var(--touch-target); padding: 0 1rem; transition: all 0.2s; cursor: pointer; }
        ...
        .person-entry, .bg-muted { background: var(--muted); border-radius: 0.75rem; padding: 1.25rem; margin-bottom: 1rem; border: 1px solid var(--border); }
        .select2-container .select2-selection--single { height: var(--touch-target) !important; border: 1px solid var(--input) !important; border-radius: 0.5rem !important; padding: 0.5rem !important; display: flex !important; align-items: center !important; }
        .select2-container--default .select2-selection--single .select2-selection__arrow { height: var(--touch-target) !important; top: 0 !important; }
        .select2-dropdown { z-index: 9999; width: 20% !important; }
        .select2-container { width: 100% !important; }
        @keyframes slide-in { from { opacity: 0; transform: translateY(10px); } to { opacity: 1; transform: translateY(0); } }
        .toast { position: fixed; top: 1rem; right: 1rem; padding: 1rem 1.5rem; background: white; border-radius: 8px; box-shadow: var(--shadow-lg); border-left: 4px solid var(--primary); z-index: 100; transform: translateX(110%); transition: transform 0.3s ease; }
...
<body>
    <div class="min-h-screen bg-background pb-10">
            ...
            <header class="app-header border-b px-4 py-3 flex items-center justify-between shadow-sm">
                <div class="header-content w-full">
                    <div class="flex items-center gap-3">
                        ...
                        <div><h1 class="text-lg font-bold text-white leading-tight">Form Pendirian CV</h1><p class="text-xs text-blue-100">Lengkapi formulir berikut untuk mengajukan pendirian CV</p></div>
                    </div>
                </div>
            </header>
            ...
            <div class="stepper-sticky bg-white/90 backdrop-blur border-b transition-all">
                <div class="stepper-content">
                    <div class="mobile-stepper">
                        <div class="mobile-stepper-header"><span class="text-foreground" id="mobile-step-title">Informasi Perusahaan</span><span class="text-muted-foreground" id="mobile-step-count">1/5</span></div>
                        <div class="mobile-progress-track"><div class="mobile-progress-fill" id="mobile-progress-bar" style="width: 20%"></div></div>
                    </div>
                    <div class="desktop-stepper">
                        <div class="stepper-line-bg"></div><div class="stepper-line-fill" id="desktop-progress-line" style="width: 0%"></div>
        ...
        <main class="main-content max-w-2xl mx-auto px-4 py-6 mb-24">
            @if(session('success'))
            <div class="mb-6 p-4 bg-green-50 border border-green-200 rounded-lg flex items-center gap-3 text-green-700"><i class="fas fa-check-circle text-xl"></i><span>{{ session('success') }}</span></div>
            @endif

            <div class="card-elevated bg-white p-6 md:p-8 rounded-xl shadow-sm border border-gray-100">
                <form action="{{ isset($pendirianCV) ? route('pendirian.cv.update', $pendirianCV->id) : route('pendirian.cv.store') }}" method="POST" enctype="multipart/form-data" id="pendirian-cv-form">
                    @csrf @if(isset($pendirianCV)) @method('PUT') @endif
                    
                    <div class="form-section active" data-step="1">
                        <div class="mb-6 pb-4 border-b">
                            <h2 class="text-xl font-bold text-gray-900 flex items-center gap-2"><i class="fas fa-building text-blue-600"></i>Informasi Perusahaan</h2>
                        ...
                        <div class="form-group mb-4">
                            ...
                            <input type="text" class="form-input" id="nama_perusahaan" name="nama_perusahaan" required value="{{ old('nama_perusahaan', $pendirianCV->nama_perusahaan ?? '') }}">
                        ...
                        <div class="border-t pt-4 mt-4">
                            <h4 class="text-md font-medium text-gray-900 mb-4 flex items-center"><i class="fas fa-map-marker-alt mr-2 text-indigo-500"></i>Alamat Lengkap Perusahaan</h4>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div class="form-group"><label for="province" class="form-label required">Provinsi</label><select class="form-input" id="province" name="province" required><option value="">-- Pilih Provinsi --</option></select><div class="error-message text-red-500 text-xs mt-1" id="province-error"></div></div>
                                <div class="form-group"><label for="city" class="form-label required">Kota/Kabupaten</label><select class="form-input" id="city" name="city" required disabled><option value="">-- Pilih Kota --</option></select><div class="error-message text-red-500 text-xs mt-1" id="city-error"></div></div>
                                <div class="form-group"><label for="district" class="form-label required">Kecamatan</label><select class="form-input" id="district" name="district" required disabled><option value="">-- Pilih Kecamatan --</option></select><div class="error-message text-red-500 text-xs mt-1" id="district-error"></div></div>
                                <div class="form-group"><label for="village" class="form-label required">Desa/Kelurahan</label><select class="form-input" id="village" name="village" required disabled><option value="">-- Pilih Desa --</option></select><div class="error-message text-red-500 text-xs mt-1" id="village-error"></div></div>
                            </div>
                            <div class="form-group mt-4">
                                <label for="alamat_lengkap" class="form-label required">Alamat Lengkap</label>
                                <textarea class="form-input" id="alamat_lengkap" name="alamat_lengkap" rows="3" required placeholder="Jalan, nomor rumah, RT/RW, dll.">{{ old('alamat_lengkap', $pendirianCV->alamat_lengkap ?? '') }}</textarea>
                            ...
                            <div class="form-group mt-4">
                                ...
                                <input type="text" class="form-input" id="kode_pos" name="kode_pos" required value="{{ old('kode_pos', $pendirianCV->kode_pos ?? '') }}">
                    ...
                    <div class="form-section" data-step="2">
                        <div class="mb-6 pb-4 border-b">
                            <h2 class="text-xl font-bold text-gray-900 flex items-center gap-2"><i class="fas fa-user-tie text-blue-600"></i>Informasi Direktur</h2>
                            <p class="text-sm text-gray-500 mt-1">Tambahkan informasi lengkap untuk setiap direktur perusahaan.</p>
                        </div>
                        <div id="direktur-container" class="space-y-4"></div>
                        <button type="button" class="btn btn-outline btn-sm mt-4 w-full md:w-auto border-dashed" id="add-direktur"><i class="fas fa-plus mr-2"></i>Tambah Direktur</button>
                    </div>

                    <div class="form-section" data-step="3">
                         <div class="mb-6 pb-4 border-b">
                            <h2 class="text-xl font-bold text-gray-900 flex items-center gap-2"><i class="fas fa-users text-blue-600"></i>Informasi Komisaris</h2>
                        </div>
                        <div id="komisaris-container" class="space-y-4"></div>
                        <button type="button" class="btn btn-outline btn-sm mt-4 w-full md:w-auto border-dashed" id="add-komisaris"><i class="fas fa-plus mr-2"></i>Tambah Komisaris</button>
                    </div>

                    <div class="form-section" data-step="4">
                        <div class="mb-6 pb-4 border-b">
                             <h2 class="text-xl font-bold text-gray-900 flex items-center gap-2"><i class="fas fa-list-alt text-blue-600"></i>KBLI & Bank</h2>
                        </div>
                        <div class="mb-4">
                            <label for="kbli-search-input" class="form-label">Cari KBLI</label>
                            <div class="search-container relative">
                                <input type="text" id="kbli-search-input" class="form-input pl-10" placeholder="Cari berdasarkan kode, judul, atau uraian...">
                                <i class="fas fa-search absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400"></i>
                                <div class="absolute w-full bg-white border border-gray-200 rounded-b-lg shadow-lg z-50 hidden" id="kbli-search-suggestions"></div>
                            </div>
                        </div>
                         <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 space-y-2 sm:space-y-0">
                                <div class="flex items-center space-x-2">
                                    <label for="kbli-per-page" class="text-sm text-gray-700">Tampilkan</label>
                                    <select id="kbli-per-page" class="form-input w-auto h-8 py-1"><option value="10">10</option><option value="25" selected>25</option><option value="50">50</option><option value="100">100</option></select>
                                </div>
                        ...
                        <div class="overflow-x-auto mb-4 border rounded-lg">
                            <table class="w-full text-sm text-left"><thead class="bg-gray-50 text-gray-700 font-medium"><tr><th class="p-3 border-b">KODE</th><th class="p-3 border-b">JUDUL</th><th class="p-3 border-b">URAIAN</th><th class="p-3 border-b text-center">AKSI</th></tr></thead><tbody id="kbli-results" class="bg-white divide-y divide-gray-200"></tbody></table>
                        </div>
                        <div id="kbli-pagination" class="flex justify-center mt-4"></div>
                        <div class="mt-6">
                            <h4 class="text-md font-medium text-gray-900 mb-2">KBLI yang Dipilih (<span id="selected-kbli-count">0</span>)</h4>
                            <div class="overflow-x-auto border rounded-lg"><table class="w-full text-sm text-left"><thead class="bg-blue-50 text-blue-800 font-medium"><tr><th class="p-3 border-b">KODE</th><th class="p-3 border-b">JUDUL</th><th class="p-3 border-b">URAIAN</th><th class="p-3 border-b text-center">HAPUS</th></tr></thead><tbody id="selected-kbli-body" class="bg-white divide-y divide-gray-200"></tbody></table></div>
                        </div>
                        <div class="bg-amber-50 border-l-4 border-amber-500 rounded-r-lg shadow-sm p-5 mb-6 mt-6">
                            <div class="flex items-start gap-4">
                                <div class="flex-shrink-0 text-amber-500 mt-1"><i class="fas fa-exclamation-triangle fa-lg"></i></div>
                                <div class="w-full">
                                    <h3 class="text-lg font-bold text-gray-800 mb-2">Informasi Biaya Tambahan KBLI</h3>
                                     <div class="inline-flex items-center bg-white border border-amber-200 rounded-md px-3 py-1 mb-3 shadow-sm"><span class="text-xs text-gray-500 uppercase tracking-wide font-bold mr-2">Batas Gratis</span><span class="text-sm font-bold text-amber-700">5 KBLI</span></div>
                                     <div id="kbli-doc-options" class="hidden">
                                        <div class="bg-blue-50 border border-blue-200 rounded-md p-3 mb-4 flex gap-3 items-start"><i class="fas fa-info-circle text-blue-500 mt-0.5"></i><div class="text-sm text-blue-800"><p class="font-bold mb-1">Anda telah memilih lebih dari 5 KBLI.</p><p class="leading-snug">Setiap kelebihan 1 KBLI akan dikenakan biaya tambahan.</p></div></div>
                                        <p class="text-sm font-semibold text-gray-700 mb-3 block">Pilih dokumen untuk kelebihan KBLI:</p>
                                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                            <label class="relative flex items-center p-3 border rounded-lg cursor-pointer hover:bg-amber-50 transition-colors border-gray-200 has-[:checked]:border-amber-500 has-[:checked]:bg-amber-50/50">
                                                <input type="radio" name="kbli_doc_option_radio" value="akta" class="h-4 w-4 text-amber-600 border-gray-300 focus:ring-amber-500"><div class="ml-3"><span class="block text-sm font-bold text-gray-900">Akta Saja</span><span class="block text-xs text-gray-500">Biaya: Rp15.000 / KBLI</span></div>
                                            </label>
                                            <label class="relative flex items-center p-3 border rounded-lg cursor-pointer hover:bg-amber-50 transition-colors border-gray-200 has-[:checked]:border-amber-500 has-[:checked]:bg-amber-50/50">
                                                <input type="radio" name="kbli_doc_option_radio" value="both" checked class="h-4 w-4 text-amber-600 border-gray-300 focus:ring-amber-500"><div class="ml-3"><span class="block text-sm font-bold text-gray-900">Akta + NIB</span><span class="block text-xs text-gray-500">Biaya: Rp115.000 / KBLI</span></div>
                                            </label>
                                        </div>
                                    </div>
                                    <div class="mt-4 pt-3 border-t border-amber-200/60 flex justify-between items-center"><div class="text-sm text-gray-600"><p>Kelebihan: <span id="excess-kbli-count" class="font-bold text-gray-800">0 Kode</span></p></div><div class="text-right"><p class="text-xs text-gray-500 uppercase font-bold">Total Tambahan</p><p class="text-xl font-extrabold text-amber-700" id="total-kbli-charge">Rp0</p></div></div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mt-8 border-t pt-6">
                            <h3 class="text-lg font-bold text-gray-900 mb-2 flex items-center gap-2"><i class="fas fa-university text-blue-600"></i>Pilih Rekanan Bank</h3>
                    ...
                    <div class="form-section" data-step="5">
                        <div class="mb-6 pb-4 border-b">
                            <h2 class="text-xl font-bold text-gray-900 flex items-center gap-2"><i class="fas fa-credit-card text-blue-600"></i>Pembayaran & Upload Bukti</h2>
                        </div>
                        <div class="bg-blue-50 border border-blue-200 rounded-lg p-6 mb-6"> <h4 class="text-lg font-semibold text-blue-900 mb-4">Ringkasan Pembayaran</h4> <div id="payment-summary" class="text-blue-800 space-y-2"></div> </div>
                        <div class="bg-amber-50 border border-amber-200 rounded-lg p-6 mb-6">
                            <h4 class="font-semibold text-amber-900 mb-3 flex items-center"><i class="fas fa-info-circle mr-2 text-amber-600"></i>Instruksi Pembayaran</h4>
                            <ol class="text-sm text-amber-800 space-y-2 list-decimal list-inside"><li>Transfer biaya</li><li>Simpan bukti</li><li>Upload bukti</li><li>Klik "Ajukan"</li></ol>
                        </div>
                        <div class="form-group">
                            <label class="form-label required">Bukti Pembayaran</label>
                            <div class="payment-proof-container border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:bg-gray-50 transition-colors cursor-pointer" id="payment-proof-container">
                                ...
                                <input type="file" id="payment_proof" name="payment_proof" class="hidden" accept="image/*,.pdf">
                                <button type="button" class="btn btn-primary mt-2" id="upload-payment-proof-btn"><i class="fas fa-upload mr-2"></i> Pilih File</button>
                            </div>
                            <div id="payment-proof-preview" class="mt-4 hidden"><div class="flex items-center justify-between p-4 bg-green-50 border border-green-200 rounded-lg"><div class="flex items-center"><i class="fas fa-check-circle text-green-500 text-2xl mr-3"></i><div><p class="font-medium text-gray-900" id="payment-proof-name"></p><p class="text-sm text-gray-500" id="payment-proof-size"></p></div></div><button type="button" class="text-red-500 hover:text-red-700" id="remove-payment-proof"><i class="fas fa-times-circle text-xl"></i></button></div></div>
                             <div class="error-message text-red-500 text-xs mt-1" id="payment_proof-error"></div>
                        </div>
                         <div class="bg-blue-600 text-white p-4 rounded-lg mt-6 flex gap-4"><i class="fas fa-list-check text-2xl mt-1"></i><div><h4 class="text-md font-bold mb-2">Ringkasan Pengajuan</h4><div id="submission-summary" class="text-sm space-y-1 text-blue-100"></div></div></div>
                    </div>
                </form>
            </div>
        </main>

        <div class="bottom-nav bg-white border-t p-4 fixed bottom-0 left-0 right-0 z-50 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
            <div class="bottom-nav-content max-w-2xl mx-auto flex gap-3">
                 <button type="button" id="prev-step-btn" class="btn btn-outline flex-shrink-0 min-w-[100px]" disabled><i class="fas fa-arrow-left mr-2"></i> Kembali</button>
                <button type="button" id="next-step-btn" class="btn btn-primary flex-grow shadow-md">Lanjut <i class="fas fa-arrow-right ml-2"></i></button>
            </div>
            <p class="text-xs text-center text-gray-500 mt-2 font-medium"><span id="time-estimate" class="text-[#B45309]">Mohon cek kembali form yang anda isi!</span></p>
        </div>
    </div>
    ...
    <script>
        ...
        if(isEdit && editData) {
             try { if (editData.kbli_selected) selectedKBLIs = JSON.parse(editData.kbli_selected); 
                   if (editData.selected_bank) selectedBank = editData.selected_bank; } catch(e){}
        }
        ...
        $(document).ready(function() {
            ...
            function performKBLISearch(query, page = 1, perPage = 25) {
                ...
                $.ajax({ url: `/api/kbli/search?query=${encodeURIComponent(query)}&page=${page}&per_page=${perPage}`, method: 'GET',
